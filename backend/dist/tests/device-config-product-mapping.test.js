...
async function runTests() {
    ...
    try {
        ...
        if (res.status === 200 && data.data?.length > 0) {
            waterProduct = data.data.find((p) => p.category === 'Nước uống') || data.data[0];
            gasProduct = data.data.find((p) => p.category === 'Gas') || data.data[1] || data.data[0];
    ...
    try {
        const res = await fetch(`${BASE_URL}/api/devices`, {
            ...
            body: JSON.stringify({
                macAddress: autoDevice.macAddress,
...
//# sourceMappingURL=device-config-product-mapping.test.js.map