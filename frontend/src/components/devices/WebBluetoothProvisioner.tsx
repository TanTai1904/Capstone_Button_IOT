...
export const WebBluetoothProvisioner: React.FC<WebBluetoothProvisionerProps> = ({
  ...
  const handleConnectBluetooth = async () => {
    ...
    try {
      ...
      if (err.name !== 'NotFoundError') {
        setErrorMsg(err.message || 'Không thể kết nối Bluetooth. Đảm bảo nút ESP32 đang bật và ở gần máy tính.');
      }
    }
  };
  ...
  const handleSendWifiBle = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Lỗi khi gửi cấu hình Wi-Fi qua Bluetooth. Vui lòng thử lại!');
    } finally {
      setIsWriting(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex p-1 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
        ...
        >
          <Usb className="w-4 h-4 text-sky-500" />
      ...
      {method === 'BLE' && (
        <div className="p-4 bg-cyan-500/5 dark:bg-cyan-500/10 border border-cyan-500/20 rounded-2xl space-y-3">
          ...
          {!connectedBleDevice ? (
            <button
              ...
              disabled={isScanning}
              className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold text-xs shadow-md hover:shadow-cyan-500/20 transition-all flex items-center justify-center gap-2"
            >
              {isScanning ? (
                <>
            ...
            <div className="p-3 bg-white dark:bg-slate-900 rounded-xl border border-cyan-500/30 text-xs space-y-1">
      ...
      <div className="space-y-3 p-4 bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800">
        <div>
          ...
          <input
            ...
            className="w-full px-3.5 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
          />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-3.5 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white font-medium pr-14 focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
            />
            <button
              type="button"
        ...
        {method === 'BLE' ? (
          ...
          <button
            ...
            disabled={isWriting}
            className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white font-extrabold text-xs shadow-md transition-all flex items-center justify-center gap-2"
          >
            {isWriting ? (
              <>
                <RefreshCw className="w-4 h-4 animate-spin" />
